...
pub type R = crate::R<PAD2_CONFIG_SPEC>;
#[doc = "Register `PAD2_CONFIG` writer"]
pub type W = crate::W<PAD2_CONFIG_SPEC>;
...
impl W {
    ...
    #[inline(always)]
    #[must_use]
    pub fn pad_ser_data_tx_config(&mut self) -> PAD_SER_DATA_TX_CONFIG_W<PAD2_CONFIG_SPEC> {
        PAD_SER_DATA_TX_CONFIG_W::new(self, 0)
    }
    #[doc = "Bits 10:19 - 0: drive strenght / 1: drive strenght / 2: schmitt trigger / 3: schmitt trigger / 4: rate / 5: output en(0) / 6: hold / 7: pull enable / 8: pd(0)/pu(1) / 9: input en(1)"]
    #[inline(always)]
    #[must_use]
    pub fn pad_ser_data_rx_config(&mut self) -> PAD_SER_DATA_RX_CONFIG_W<PAD2_CONFIG_SPEC> {
        PAD_SER_DATA_RX_CONFIG_W::new(self, 10)
    }
    #[doc = "Bits 20:29 - 0: drive strenght / 1: drive strenght / 2: schmitt trigger / 3: schmitt trigger / 4: rate / 5: output en(0) / 6: hold / 7: pull enable / 8: pd(0)/pu(1) / 9: input en(1)"]
    #[inline(always)]
    #[must_use]
    pub fn pad_ser_clk_config(&mut self) -> PAD_SER_CLK_CONFIG_W<PAD2_CONFIG_SPEC> {
